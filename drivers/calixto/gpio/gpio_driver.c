...
static long gpio_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	if (_IOC_TYPE(cmd) != DEXCEL_IOC_MAGIC)
		return -ENOTTY;
...
static irqreturn_t gpio_irq_handler(int irq, void *dev_id) {
	...
	struct evm_gpio_data *gpdev = dev_id;

	if (gpdev->timer_debounce)
                mod_timer(&gpdev->timer,
                        jiffies + msecs_to_jiffies(gpdev->timer_debounce));

	printk("gpio button pressed\n");

   return IRQ_HANDLED;
}

static void gpio_irq_timer(unsigned long _data)
{
        struct evm_gpio_data *gpdev = (struct evm_gpio_data *)_data;
        gpdev->interrupt = true;
}
	...
	if (IS_ERR(gpdev->dev)) {
	  ...
	  for (i = 0; i < gpio_pdata->nbuttons; i++){
		      ...
		      status = gpio_request(buttons[i].gpio, buttons[i].name);
				...
				error = gpio_direction_input(buttons[i].gpio);
	  				 if (error < 0) {
                				printk("Failed to configure"
                        			     " direction for GPIO ID-> %d, Name-> %s\n", buttons[i].id, buttons[i].name);
				 	 }

		             if (buttons[i].irq_enable == ENABLE){
					
				irqflags = IRQF_DISABLED | IRQF_TRIGGER_FALLING;

			           if (buttons[i].debounce_interval){
                        		gpdev->timer_debounce = buttons[i].debounce_interval;
        				gpdev->gpio_irq = gpio_to_irq(buttons[i].gpio);
				   	  	
						printk("Inside debounce_interval\n");
	
					    if (gpdev->gpio_irq < 0) {
                			           error = gpdev->gpio_irq;
						   printk("Failed to configure as a IRQ GPIO-> %d\n", error);
					    }

					     error = request_irq(gpdev->gpio_irq, gpio_irq_handler,
											irqflags, buttons[i].name, gpdev);
        					if(error < 0) {
							printk("Unable to claim irq %d; error %d\n", gpdev->gpio_irq, error);
				   		}
				    }
			     }
			}		
